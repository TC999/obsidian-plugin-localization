...
async function getTemplateInfo(template) {
    ...
    try {
        ...
        console.error(`未找到自定义每周笔记设置！'${templatePath}'`, err);
        new obsidian__default['default'].Notice("阅读每日笔记模板失败");
        return ["", null];
    }
}
...
async function createDailyNote(date) {
    ...
    try {
        ...
        console.error(`文件创建失败：'${normalizedPath}'`, err);
...
function getAllDailyNotes() {
    ...
    if (!dailyNotesFolder) {
        throw new DailyNotesFolderMissingError("找不到每日笔记文件夹");
    }
    const dailyNotes = {};
...
function getAllWeeklyNotes() {
    ...
    if (!weeklyNotesFolder) {
        throw new WeeklyNotesFolderMissingError("找不到每周笔记文件夹");
    }
    const weeklyNotes = {};
...
async function openPeriodicNote(periodicity, date, inNewSplit) {
    ...
    try {
        ...
        console.error(`没有找到你的 ${periodicity} 笔记文件夹`, err);
        new obsidian.Notice(`没有找到你的 ${periodicity} 笔记文件夹`);
        return;
    }
...
function showFileMenu(app, settings, position) {
    ...
    ["daily", "weekly", "monthly"]
        ...
        contextMenu.addItem((item) => item
            .setTitle(`打开 ${config.relativeUnit}`)
...
function create_if_block_3(ctx) {
	...
	return {
		c() {
			...
			div0 = element("div");
			h4 = element("h4");
			h4.textContent = "Daily Notes 插件已启用";
			t1 = space();
			if_block0.c();
			t2 = space();
			div1 = element("div");
...
function create_else_block_2(ctx) {
	...
	return {
		c() {
			p = element("p");
			p.textContent = "您目前正在使用核心Daily Notes插件。您可以将这些设置迁移到定期笔记，以享受相同的功能和一些显著的改进";
...
function create_if_block_5(ctx) {
	...
	return {
		c() {
			...
			p.innerHTML = `您已经成功迁移了每日笔记设置。你可以现在禁用Daily Notes核心插件，以避免任何混乱。<br/>如果你有一个自定义热键的日常笔记，确保更新让他们使用新的“定期笔记”。命令。`;
...
function create_if_block_1$2(ctx) {
	...
	return {
		c() {
			...
			div0.innerHTML = `<h4>迁移每周笔记设置</h4> 
        <p class="setting-item-description">您现有的日历插件的每周笔记设置在自动迁移。该功能将被删除在未来的日历插件。</p>`;
...
class PeriodicNotesPlugin extends obsidian.Plugin {
    ...
    configureRibbonIcons() {
        ...
        if (configuredPeriodicities.length) {
            ...
            const config = periodConfigs[periodicity];
            this.ribbonEl = this.addRibbonIcon(`calendar-${config.unitOfTime}`, `打开 ${config.relativeUnit}`, (event) => openPeriodicNote(periodicity, window.moment(), isMetaPressed(event)));
