...
class ActivityHistorySettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        containerEl.empty();
        containerEl.createEl('h2', { text: '活动历史设置' });
        new obsidian.Setting(containerEl)
            .setName('已跟踪的项目:')
            .setDesc(this.plugin.settings.trackedProjects.join(" ---------  "));
        new obsidian.Setting(containerEl)
            .setName('添加跟踪项目:')
            .setDesc('将项目添加到跟踪列表。例如，写“项目1”以跟踪位于“/项目1”中的项目')
            .addText(text => text
            .setPlaceholder(`项目实例`)
        ...
        new obsidian.Setting(containerEl)
            .setName('Cell 半径:')
            .setDesc('Cell 半径。整数，默认是 1=方形，越高越圆。')
            .addText(text => text
