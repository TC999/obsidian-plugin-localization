...
class CalendarSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        if (!appHasDailyNotesPluginLoaded_1()) {
            this.containerEl.createDiv("settings-banner", (banner) => {
                ...
                banner.createEl("p", {
                    ...
                    text: "日历最好与Daily Notes插件或Periodic Notes插件(可在Community Plugins目录中获得)一起使用。",
                });
            });
        }
        ...
        if (this.plugin.options.showWeeklyNote &&
            ...
            this.containerEl.createEl("p", {
                cls: "setting-item-description",
                text: "注意:每周的笔记设置正在移动。我们鼓励您安装“定期笔记”插件，以保持功能在未来。",
            });
    ...
    addWeekStartSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("每周初始日:")
            .setDesc("选择从一周的哪一天开始。选择'区域默认'以使用 moment.js 指定的默认值")
            .addDropdown((dropdown) => {
            dropdown.addOption("locale", `区域默认 (${localeWeekStart})`);
    ...
    addLocaleOverrideSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("区域选择:")
            .setDesc("如果您想使用不同于默认的区域设置，请设置此参数")
            .addDropdown((dropdown) => {
            dropdown.addOption("system-default", `与系统一样的 (${sysLocale})`);
...
async function tryToCreateDailyNote(date, inNewSplit, settings, cb) {
    ...
    if (settings.shouldConfirmBeforeCreate) {
        createConfirmationDialog({
            cta: "确定",
            onAccept: createFile,
            text: `您想新建一个文件名为 “${filename}” 的 每日笔记 吗?`,
            title: "新建 每日笔记",
        });
    }
    else {
...
async function tryToCreateWeeklyNote(date, inNewSplit, settings, cb) {
    ...
    if (settings.shouldConfirmBeforeCreate) {
        createConfirmationDialog({
            cta: "确定",
            onAccept: createFile,
            text: `您想新建一个文件名为 “${filename}” 的 每周笔记 吗?`,
            title: "新建 每周笔记",
        });
    }
    else {
